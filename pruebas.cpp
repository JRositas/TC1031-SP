...
int main(){
    ...
    int contadorEntrada, fechaCode, numEntradas;
    string fecha, ubi, hora, codigoBusq;
    char puntoEntrada;
    ...
    while (archivoSuez >> fecha >> hora >> puntoEntrada >> ubi)
    {
        ...
        mivect[numEntradas] = *objAuxiliar;  //push_back() es el metodo que guarda los valore de un vector en el
        numEntradas++;
    }

    cin >> codigoBusq;
    busqSeceuncia(mivect,codigoBusq);
}